...
def discrete_obstacles_terrain(terrain, max_height, min_size, max_size, num_rects, platform_size=1.):
    ...
    y1 = (terrain.length - platform_size) // 2
    y2 = (terrain.length + platform_size) // 2
    terrain.height_field_raw[x1:x2, y1:y2] = 0 ### 原本是0
    return terrain
...
def discrete_obstacles_terrain_cells1(
    ...
    (i, j) = terrain.height_field_raw.shape
    # print('discrete_obstacles_terrain_cells')
    for _ in range(num_rects):
    ...
    y1 = (terrain.length - platform_size) // 2
    y2 = (terrain.length + platform_size) // 2
    terrain.height_field_raw[x1:x2, y1:y2] = 10
    return terrain
...
def discrete_obstacles_terrain_cells(
    ...
    # print(min_size)
    # print(max_size)
    width_range = range(min_size, max_size, 1)
    length_range = range(min_size, max_size, 1)
    (i, j) = terrain.height_field_raw.shape
    # print('114514          discrete_obstacles_terrain_cells')
    for _ in range(num_rects):
        width = np.random.choice(width_range)
        length = np.random.choice(length_range)
        ...
        # start_j = np.random.choice(range(0, j - length, 1))
        # height = min_height + np.random.rand() * (max_height - min_height)
        # terrain.height_field_raw[start_i : start_i + width, start_j : start_j + length] = p.random.choice(height_range)
        # (
        #     height / terrain.vertical_scale
        # )
        # print("^^^^^^^^^^^^^^^^^^^^")
        ...
        # print("start_j")
        # print(start_j)
        # print("start_j + length")
        # print(start_j + length)
        # print("height / terrain.vertical_scale")
        # print("^^^^^^^^^^^^^^^^^^^^")
    x1 = (terrain.width - platform_size) // 2
    x2 = (terrain.width + platform_size) // 2
    y1 = (terrain.length - platform_size) // 2
...
def discrete_obstacles_terrain_cells1(
    ...
    jump = 10
    H = [200, 300, 400, 600]
    for start_i in range(0, i - 4 * 2 - jump - 1, 4 * 15):
        for start_j in range(0, j - 4 * 2 - jump - 1, 4 * 15):
            hh = choice(H)    
            length = choice(L) 
            # print('start_i, start_i + width, start_j, start_j + length', start_i, start_i + width, start_j, start_j + length)
            # print(hh)
            terrain.height_field_raw[start_i + length + jump : start_i + length + length + jump, start_j + length + jump : start_j + length + length + jump] = 200
            terrain.height_field_raw[start_i + length + jump : start_i + length + length + jump, start_j : start_j + length] = 200
            terrain.height_field_raw[start_i : start_i + length, start_j + length + jump : start_j + length + length + jump] = 200
            terrain.height_field_raw[start_i : start_i + length, start_j : start_j + length] = 200
    ...
    # for _ in range(num_rects):
        ...
        #     height / terrain.vertical_scale
        # )        
    # print('height / terrain.vertical_scale', height / terrain.vertical_scale)
    # print('terrain.height_field_raw', terrain.height_field_raw)
    # print('terrain.width, terrain.length, platform_size', terrain.width, terrain.length, platform_size)    
    # # print('terrain.height_field_raw', terrain.height_field_raw.size)
    # print('x1 x2 y1 y2:', x1, x2, y1, y2)
    # print('i, j', i, j)
    # for u in range(0 , i):
    #     for v in range(0, j): 
    #         print(terrain.height_field_raw[u][v], end=' ')
    #     print('')
